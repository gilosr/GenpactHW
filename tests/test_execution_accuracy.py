...
class TestCompareResultSets:
    def test_identical_results_match(self):
        ...
        assert compare_result_sets(a, b).get("is_match") is True
    ...
    def test_different_values_do_not_match(self):
        a = [{"count": 9}]
        b = [{"count": 15}]
        assert compare_result_sets(a, b).get("is_match") is False
    ...
    def test_empty_results_match(self):
        assert compare_result_sets([], []).get("is_match") is True
